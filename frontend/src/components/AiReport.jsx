...
import { Sparkles, ChevronDown, Copy, Check, RefreshCw } from 'lucide-react'
...
const REPORT_TYPES = [
  ...
  {
    id:    'partenaires',
    label: 'Rapport partenaires',
    desc:  'Draft prêt à envoyer aux sponsors et partenaires',
    icon:  '🤝',
  },
]
...
function renderMarkdown(text) {
  return text
    ...
    .replace(/\*(.+?)\*/g, '<em>$1</em>')
    .replace(/^### (.+)$/gm, '<h4 style="color:#F0F4FF;font-weight:700;margin:1rem 0 .25rem">$1</h4>')
    .replace(/^## (.+)$/gm,  '<h3 style="color:#F0F4FF;font-weight:700;font-size:1rem;margin:1.25rem 0 .375rem">$1</h3>')
    .replace(/^# (.+)$/gm,   '<h2 style="color:#F0F4FF;font-weight:800;font-size:1.125rem;margin:1.5rem 0 .5rem">$1</h2>')
    .replace(/\n/g, '<br>')
}
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          {REPORT_TYPES.map(r => (
            <button
              ...
              <div>
                <p className="text-xs font-semibold" style={{ color: selectedType === r.id ? '#A5B4FC' : '#F0F4FF' }}>
                  {r.label}
                </p>
                <p className="text-2xs mt-0.5 leading-snug" style={{ color: '#4A5568', fontSize: '0.575rem' }}>
                  {r.desc}
                </p>
              </div>
            </button>
        ...
        <button
          ...
          {loading
            ...
            : <><Sparkles size={15} strokeWidth={2} /> Générer — {currentType?.label}</>
          }
        </button>

        {/* Erreur */}
        {error && (
          <div className="p-3 rounded-xl text-xs" style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', color: '#FCA5A5' }}>
          ...
          <div className="rounded-xl p-4 text-sm leading-relaxed"
            ...
            {text ? (
              ...
              <div className="flex items-center gap-2" style={{ color: '#4A5568' }}>
                ...
                <span className="text-xs">Claude analyse vos données…</span>
              </div>
            )}
            {/* Curseur clignotant pendant génération */}
            {loading && <span className="inline-block w-0.5 h-4 ml-0.5 animate-pulse" style={{ background: '#6366F1', verticalAlign: 'middle' }} />}
          </div>
        )}
      </div>
    </div>
